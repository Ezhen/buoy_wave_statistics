...
# repo root is one level up from tools/ - resolve the sibling stage
# script by absolute path so this still works regardless of the
# caller's cwd, rather than relying on "must be run from repo root"
# (pipeline_out/ and data_multiyear/ paths below are still CWD-relative,
# matching every other stage script's convention - only the sibling
# script invocation needed hardening here).
REPO_ROOT = Path(__file__).resolve().parent.parent
STAGE25_SCRIPT = REPO_ROOT / "25_changepoint_detection.py"
...
def run_stage25(buoy: str, var: str, start_year=None):
    cmd = [sys.executable, str(STAGE25_SCRIPT), "--buoy", buoy, "--var", var]
